...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        for i, (p, dp) in enumerate(self.params):
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            self.curr_grad[i] = self.momentum * self.curr_grad[i] - dp * self.learn_rate
            p += self.curr_grad[i]
            # ========================
...
class RMSProp(Optimizer):
    ...
    def step(self):
        for i, (p, dp) in enumerate(self.params):
            ...
            # parameters tensor.
            # ====== YOUR CODE: ======
            self.r_vector[i] = self.decay * self.r_vector[i] + (1 - self.decay) * (torch.square(dp))
            down = torch.sqrt(self.r_vector[i] + self.eps)
            up = self.learn_rate
            term = up/down
            p -= term * dp
            dp += self.reg * p
